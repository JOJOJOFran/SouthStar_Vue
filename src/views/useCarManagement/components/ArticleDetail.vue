          ...
          <div ref="print" style="width:100%;height:100%;" id="printContent">
            ...
            <tr>
              ...
              <td id="carType" style="text-align:center;font-weight:bold;font-family:'STKaiti';">
                <el-col :span="24">
                  <el-select v-model="addParam.carType" class="filter-item" placeholder="选择派车种类">
      ...
      vehicleEnableList().then(response => {
        ...
        for(var i=0;i< count;i++){
          if(this.vehicleList[i].desc.toLowerCase().indexOf(queryString.toLowerCase()) !=-1){
            if(!this.postForm.comment_disabled && this.vehicleList[i].desc.toLowerCase().indexOf("公务用车") !=-1)//公务用车
            {
              carData.push({value:this.vehicleList[i].desc,plateNumber:this.vehicleList[i].plateNumber,vehicleId:this.vehicleList[i].id});
            }
            else if(this.postForm.comment_disabled && this.vehicleList[i].desc.toLowerCase().indexOf("应急执法") !=-1)
            {
              carData.push({value:this.vehicleList[i].desc,plateNumber:this.vehicleList[i].plateNumber,vehicleId:this.vehicleList[i].id});
            }
          }
